...
// The adapter copies preview credentials. They must never remain in build artifacts.
for (const file of await readdir(new URL('../dist/server/', import.meta.url))) {
  if (/^\.(dev\.vars|env)(\.|$)/.test(file)) {
    await rm(new URL(`../dist/server/${file}`, import.meta.url));
  }
}
