...
## Form delivery through Cloudflare Email Routing, 2026-09-25

The owner selected Cloudflare forwarding and changed the form destination. Cloudflare's current documentation explicitly allows free sending to verified Email Routing destinations, including accounts that have only Routing enabled. The earlier paid-plan blocker applied to arbitrary recipients; it does not apply to this fixed, verified inbox. The existing domain's Routing status was confirmed as ready, and a verification email was requested for the new destination. The owner explicitly approved adding only `email_routing:write` to the existing account-specific OAuth profile after automatic approval review required that consent.

The native `send_email` binding uses the current structured message API, avoiding an extra MIME library or delivery API key. Runtime secrets hold the fixed destination and sender; the visitor controls neither. Reply-To contains the validated visitor address. Both HTML and plain text preserve the enquiry, with HTML escaping for visitor content. The handler awaits a nonempty Cloudflare message ID before returning success and keeps delivery errors generic so addresses and message contents cannot leak through responses or logs. No automatic retry is attempted after an uncertain delivery.

Binding configuration does not embed the recipient because the brief forbids committing contact values. Cloudflare limits this binding to verified account destinations; application code further fixes the recipient to its secret value. Local previews force simulation with synthetic addresses, while delivery acceptance, failed sends, invalid configuration and HTML injection are covered by tests.

Sources: [Cloudflare pricing](https://developers.cloudflare.com/email-service/platform/pricing/), [send bindings](https://developers.cloudflare.com/email-service/configuration/send-bindings/), [Workers API](https://developers.cloudflare.com/email-service/api/send-emails/workers-api/), and [local email simulation](https://developers.cloudflare.com/email-service/local-development/sending/).
